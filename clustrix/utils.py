            ...
            def visit_For(self, node):
                ...
                if isinstance(node.target, ast.Name):
                    loop_info = {
                        ...
                        'iterable': ast.unparse(node.iter) if hasattr(ast, 'unparse') else 'unknown'
                    }
            ...
            if loop['type'] == 'for' and 'range(' in loop['iterable']:
                ...
                try:
                    ...
                    if 'range(' in range_str:
                        range_part = range_str[range_str.find('range('):range_str.find(')', range_str.find('range(')) + 1]
                        range_obj = eval(range_part)  # Dangerous in practice, needs safer evaluation
...
def setup_environment(work_dir: str, requirements: Dict[str, str], config: ClusterConfig) -> str:
    """
    ...
    if requirements:
        ...
        req_content = '\n'.join([f"{pkg}=={version}" for pkg, version in requirements.items()])
...
def create_job_script(cluster_type: str, job_config: Dict[str, Any], remote_job_dir: str, config: ClusterConfig) -> str:
...
def _create_slurm_script(job_config: Dict[str, Any], remote_job_dir: str, config: ClusterConfig) -> str:
    """Create SLURM job script."""
    
    script_lines = [
        "#!/bin/bash",
        f"#SBATCH --job-name=clusterpy",
...
def _create_pbs_script(job_config: Dict[str, Any], remote_job_dir: str, config: ClusterConfig) -> str:
...
def _create_ssh_script(job_config: Dict[str, Any], remote_job_dir: str, config: ClusterConfig) -> str:
