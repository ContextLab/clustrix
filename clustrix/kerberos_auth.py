...
class KerberosAuthMethod(AuthMethod):
    ...
    def is_applicable(self, connection_params: Dict[str, Any]) -> bool:
        ...
        return any(
            hostname.endswith(cluster) or hostname == cluster
    ...
    def attempt_auth(self, connection_params: Dict[str, Any]) -> AuthResult:
        ...
        if not self._has_valid_ticket():
            return AuthResult(
                ...
                guidance=self._get_kinit_guidance(
                    str(cluster_config["realm"]), username
                ),
            )
        ...
        try:
            ...
            real_hostname = str(cluster_config["hostname"])
...
"""
    ...
    def _ensure_ssh_config(self, hostname: str, cluster_config: Dict[str, Any]):
        ...
        if (
            "GSSAPIAuthentication yes" in existing_config
            and "GSSAPIDelegateCredentials yes" in existing_config
        ):
    ...
    def _ensure_krb5_config(self):
        ...
        if krb5_config_path.exists():
            try:
                ...
                if (
                    "KIEWIT.DARTMOUTH.EDU" in config_content
                    and "aes256-cts-hmac-sha1-96" in config_content
                ):
...
"""
    ...
    def _get_troubleshooting_guidance(self, error_msg: str) -> str:
        ...
        if "GSS" in error_msg or "GSSAPI" in error_msg:
            ...
            guidance.append(
                "• Verify SSH config includes GSSAPIAuthentication and GSSAPIDelegateCredentials"
            )

        if "hostname" in error_msg.lower() or "connection" in error_msg.lower():
            guidance.append(
                "• Ensure you're using the correct hostname (some clusters require real hostname)"
            )
        ...
        if "encryption" in error_msg.lower() or "deprecated" in error_msg.lower():
            guidance.append(
                "• Update krb5.conf with modern encryption types (see Kerberos setup)"
            )
...
def setup_kerberos_for_cluster(hostname: str, username: str) -> bool:
    ...
    connection_params = {"hostname": hostname, "username": username, "port": 22}
