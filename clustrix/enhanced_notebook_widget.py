...
from .validation import validate_cluster_auth, validate_ssh_key_auth, validate_1password_integration
...
def create_enhanced_cluster_widget(config: ClusterConfig = None) -> widgets.VBox:
    ...
    if not IPYTHON_AVAILABLE:
        raise ImportError("IPython and ipywidgets are required for the widget interface")
    ...
    cluster_type = widgets.Dropdown(
        options=['local', 'ssh', 'slurm', 'pbs', 'sge', 'kubernetes', 'aws', 'azure', 'gcp'],
        ...
        with status_output:
            ...
            if widget_config.use_1password or widget_config.use_env_password:
                ...
                if widget_config.use_1password:
                    note_name = widget_config.onepassword_note or f"clustrix-{widget_config.cluster_host}"
                    print(f"    • 1Password (note: {note_name})")
                if widget_config.use_env_password:
                    print(f"    • Environment variable: ${widget_config.password_env_var}")
            ...
            if not password:
                ...
                print("   Authentication chain exhausted - please check your configuration")
                return
            ...
            try:
                ...
                if result:
                    ...
                    if validate_ssh_key_auth(widget_config):
                        ...
                        print("   Keys may need time to propagate or cluster may have additional requirements")
                else:
