...
class ClusterExecutor:
    ...
    def _setup_ssh_connection(self):
        ...
        connect_kwargs = {'hostname': self.config.cluster_host, 'port': self.config.cluster_port}
        
        if self.config.key_file:
            connect_kwargs['key_filename'] = self.config.key_file
        elif self.config.username and self.config.password:
            connect_kwargs.update({'username': self.config.username, 'password': self.config.password})
        else:
    ...
    def _setup_kubernetes(self):
        ...
        try:
            ...
            raise ImportError("kubernetes package required for Kubernetes cluster support")
    ...
    def _submit_slurm_job(self, func_data: Dict[str, Any], job_config: Dict[str, Any]) -> str:
        ...
        # Setup environment
        setup_remote_environment(self.ssh_client, remote_job_dir, func_data['requirements'])
    ...
    def _submit_pbs_job(self, func_data: Dict[str, Any], job_config: Dict[str, Any]) -> str:
        """Submit job via PBS."""
    ...
    def _submit_ssh_job(self, func_data: Dict[str, Any], job_config: Dict[str, Any]) -> str:
        ...
        if self.config.cluster_type == 'slurm':
            ...
            try:
                ...
                if not stdout.strip():
                    ...
                    result_exists = self._remote_file_exists(f"{job_info['remote_dir']}/result.pkl")
            ...
            if result_exists:
                ...
                try:
                    stdout, _ = self._execute_remote_command(f"wc -l {job_info['remote_dir']}/job.err")
    ...
    def _get_error_log(self, job_id: str) -> str:
        ...
        for error_file in error_files:
            try:
                stdout, _ = self._execute_remote_command(f"cat {remote_dir}/{error_file} 2>/dev/null")
                if stdout.strip():
                    return stdout
            except:
                continue
