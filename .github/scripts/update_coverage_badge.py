...
It reads the total coverage percentage (including branch coverage when available) and updates 
the README.md badge with the appropriate color based on quality thresholds:
- red < 50%
- orange < 60%  
- yellow < 70%
- yellowgreen < 80%
...
Requires:
    - coverage.json file in the project root (generated by pytest --cov-report=json --cov-branch)
...
def get_coverage_percentage(coverage_file_path):
    ...
    try:
        ...
        # Also get branch coverage if available
        branch_coverage = coverage_data['totals'].get('percent_covered_display', None)
        
        print(f"📊 Line coverage: {total_coverage:.2f}%")
