...
def test_level2_complexity():
    ...
    def complex_level2():
        ...
        results["step1"] = {"success": result1.returncode == 0, "output": result1.stdout.strip()}
        
        # Step 2
        result2 = subprocess.run(
            ["python", "-c", "import torch; print(f'GPU_AVAILABLE:{torch.cuda.is_available()}')"],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            universal_newlines=True,
            timeout=30
        )
        results["step2"] = {"success": result2.returncode == 0, "output": result2.stdout.strip()}
    ...
    def complex_level4():
        ...
        try:
            ...
            env_result = subprocess.run(
                ["python", "-c", "import os; print(f'CUDA_ENV:{os.environ.get(\"CUDA_VISIBLE_DEVICES\", \"none\")}')"],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                universal_newlines=True,
                timeout=15
        ...
        try:
            ...
            for i, cmd in enumerate(gpu_commands):
                ...
                gpu_results[f"cmd_{i}"] = {
                    ...
                    "output": gpu_proc.stdout.strip() if gpu_proc.returncode == 0 else gpu_proc.stderr
                }
        ...
        try:
            ...
            for test_name, test_code in compute_tests.items():
                ...
                compute_results[test_name] = {
                    ...
                    "output": comp_proc.stdout.strip() if comp_proc.returncode == 0 else comp_proc.stderr,
                    "execution_time": time.time()
                }
            
            phases["phase3_compute"] = {
                "success": sum(1 for r in compute_results.values() if r["success"]) >= 2,
        ...
        try:
            ...
            # Count successes
            successful_phases = sum(1 for phase in phases.values() if phase and phase.get("success", False))
            ...
            if phases["phase3_compute"] and phases["phase3_compute"]["success"]:
                ...
                compute_success_count = sum(1 for test in compute_data.values() if test["success"])
            ...
            analysis = {
                ...
                "overall_success": successful_phases >= 3 and compute_success_count >= 2,
        ...
        return {
            "phases": phases,
            "metadata": metadata,
            "final_success": phases["phase4_analysis"]["data"]["overall_success"] if phases["phase4_analysis"] and phases["phase4_analysis"]["success"] else False
        }
    
    print("Testing level 4 complexity...")
    try:
        result = complex_level4()
        print(f"✅ Level 4 completed! Final success: {result.get('final_success', 'unknown')}")
...
def test_slurm_complexity():
    ...
    def slurm_complex_test():
        ...
        try:
            env_result = subprocess.run(
                ["python", "-c", "import os; print(f'SLURM_JOB_ID:{os.environ.get(\"SLURM_JOB_ID\", \"none\")}')"],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                universal_newlines=True,
                timeout=15
            )
            workflow["step1_env"] = {"success": env_result.returncode == 0, "output": env_result.stdout.strip()}
        except Exception as e:
        ...
        try:
            ...
            for cmd in sys_commands:
                ...
                sys_results[cmd] = sys_proc.stdout.strip() if sys_proc.returncode == 0 else "failed"
        ...
        try:
            ...
            for i, check in enumerate(python_checks):
                ...
                python_results[f"check_{i}"] = {
                    ...
                    "output": py_proc.stdout.strip() if py_proc.returncode == 0 else py_proc.stderr
                }
        ...
        # Summary
        successful_steps = sum(1 for step in workflow.values() if step and step.get("success", False))
