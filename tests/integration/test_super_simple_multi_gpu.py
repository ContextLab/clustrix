...
def test_super_simple_multi_gpu():
    ...
    def simple_multi_gpu_test():
        ...
        result = subprocess.run(
            [
                "python",
                "-c",
                "import torch; print(f'GPUS:{torch.cuda.device_count()}'); x=torch.tensor([1.0]).cuda(); print(f'GPU0:ok'); torch.cuda.set_device(1); y=torch.tensor([2.0]).cuda(); print(f'GPU1:ok')",
            ],
    ...
    try:
        ...
        if "GPUS:" in output and "GPU0:ok" in output and "GPU1:ok" in output:
            ...
            gpu_count = int(output.split("GPUS:", 1)[1].split("\n")[0])
            print(
                f"⚠️  GPUs detected: {gpu_count}, but multi-GPU access may have failed"
            )
            return False
