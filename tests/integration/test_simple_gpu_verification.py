        ...
        if result["success"]:
            ...
            if "GPU_COUNT:" in output:
                ...
                if gpu_count == 8:
                    ...
                    print(
                        "✅ Configuration fix worked - CUDA_VISIBLE_DEVICES restriction removed"
                    )
                    return True
                elif gpu_count > 1:
                    print(f"⚠️  Detected {gpu_count} GPUs (expected 8)")
                    print(
                        "📋 This may be due to SLURM job allocation or other restrictions"
                    )
                    return False
                elif gpu_count == 1:
                    print(
                        "❌ Still only detecting 1 GPU - config fix may not have taken effect"
                    )
                    return False
                else:
