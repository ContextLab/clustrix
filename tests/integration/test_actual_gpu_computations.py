...
def test_actual_gpu_computation():
    ...
    try:
        ...
        for i, res in enumerate(result["results"]):
            ...
            print(
                f"     Eigenvalue range: [{res['min_eigenval']:.6f}, {res['max_eigenval']:.6f}]"
            )
            ...
            if not res["eigenvals_all_nonneg"]:
                print(
                    f"     ❌ Non-positive eigenvalue detected: {res['min_eigenval']}"
                )
...
def test_gpu_parallel_computation():
    ...
    try:
        ...
        if not result.get("success", False):
            print(
                f"❌ Parallel GPU computation failed: {result.get('error', 'Unknown error')}"
            )
            return False
        ...
        for res in result["results"]:
            ...
            print(
                f"   Task {task_id}: {res['matrix_size']}x{res['matrix_size']} on {device}"
            )
