...
def test_multi_gpu_simple():
    ...
    configure(
        ...
        job_poll_interval=5,
        environment_variables={"CUDA_VISIBLE_DEVICES": "0,1"},  # Just 2 GPUs
    )
    ...
    def simple_two_gpu_check():
        ...
        result = subprocess.run(
            [
                "python",
                "-c",
                "import torch; import os; print(f'ENV:{os.environ.get(\"CUDA_VISIBLE_DEVICES\", \"none\")}'); print(f'COUNT:{torch.cuda.device_count()}'); a=torch.tensor([1.0]).cuda(0); b=torch.tensor([2.0]).cuda(1); print(f'GPU0:{a.device}'); print(f'GPU1:{b.device}')",
            ],
