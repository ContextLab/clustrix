        ...
        try:
            gpu_check = subprocess.run(
                [
                    "python",
                    "-c",
                    "import torch; import os; print(f'ENV:{os.environ.get(\"CUDA_VISIBLE_DEVICES\", \"default\")}'); print(f'COUNT:{torch.cuda.device_count()}')",
                ],
...
if device_count > 1:
    ...
    print("SINGLE_GPU_ONLY")
            ...
            if parallel_result.returncode == 0:
                ...
                result["data_parallel_test"] = {
                    "success": False,
                    "error": parallel_result.stderr,
                }
        except Exception as e:
            ...
            if distributed_result.returncode == 0:
                ...
                for line in lines:
                    if line.startswith("AVAILABLE_DEVICES:"):
                        distributed_info["available_devices"] = int(
                            line.split(":", 1)[1]
                        )
                ...
                result["distributed_computation"] = {
                    "success": True,
                ...
                result["distributed_computation"] = {
                    "success": False,
                    "error": distributed_result.stderr,
                }
        except Exception as e:
    ...
    try:
        ...
        if result["distributed_computation"]["success"]:
            ...
            if "insufficient_gpus" in info:
                ...
                print(
                    f"Distributed computation: Success across {info['available_devices']} devices"
                )
                print(f"Result shape: {info['result_shape']}")
                print(f"Result sum: {info['result_sum']:.2f}")
        else:
            print(
                f"Distributed computation failed: {result['distributed_computation']['error']}"
            )
