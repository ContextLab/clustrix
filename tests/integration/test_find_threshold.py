...
def test_level5_higher_complexity():
    ...
    def complex_level5():
        ...
        try:
            ...
            env_checks["python_path"] = (
                path_result.stdout.strip() if path_result.returncode == 0 else "unknown"
            )
            ...
            env_checks["torch_version"] = (
                torch_result.stdout.strip()
        ...
        try:
            ...
            gpu_analysis["cuda_available"] = (
                cuda_result.stdout.strip() == "True"
            ...
            if gpu_analysis["device_count"] > 0:
                for i in range(
                    min(gpu_analysis["device_count"], 2)
                ):  # Limit to 2 devices to avoid complexity
                    name_result = subprocess.run(
                        [
                            "python",
                            "-c",
                            f"import torch; print(torch.cuda.get_device_name({i}))",
                        ],
                        stdout=subprocess.PIPE,
                        stderr=subprocess.PIPE,
                        universal_newlines=True,
        ...
        try:
            ...
            computation_tests["basic_math"] = {
                ...
                "output": (
                    math_result.stdout.strip()
                    if math_result.returncode == 0
                    else math_result.stderr
                ),
            }
            ...
            tensor_result = subprocess.run(
                [
                    "python",
                    "-c",
                    "import torch; x=torch.tensor([1.0, 2.0]).cuda(); print(f'TENSOR_SUM:{x.sum().item()}')",
                ],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                universal_newlines=True,
            ...
            computation_tests["gpu_tensor"] = {
                ...
                "output": (
                    tensor_result.stdout.strip()
                    if tensor_result.returncode == 0
                    else tensor_result.stderr
                ),
            }
            ...
            matrix_result = subprocess.run(
                [
                    "python",
                    "-c",
                    "import torch; a=torch.randn(10,10).cuda(); b=torch.mm(a,a.t()); print(f'MATRIX_TRACE:{b.trace().item():.2f}')",
                ],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                universal_newlines=True,
            ...
            computation_tests["matrix_ops"] = {
                ...
                "output": (
                    ...
                    else matrix_result.stderr
                ),
            }

            workflow["phase3_computation"] = {
                "success": True,
                "data": computation_tests,
            }
...
""",
                ...
                if timing_result.returncode == 0:
                    ...
                    performance_data["timing_tests"].append(
                        {"size": size, "success": False, "error": timing_result.stderr}
                    )
            ...
            memory_result = subprocess.run(
                [
                    "python",
                    "-c",
                    "import torch; print(f'MEMORY_ALLOCATED:{torch.cuda.memory_allocated()/1024/1024:.1f}MB') if torch.cuda.is_available() else print('NO_CUDA')",
                ],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                universal_newlines=True,
        ...
        try:
            ...
            if (
                workflow["phase3_computation"]
                and workflow["phase3_computation"]["success"]
            ):
                comp_data = workflow["phase3_computation"]["data"]
                computation_success = sum(
                    1
                    for test in comp_data.values()
            ...
            if workflow["phase4_analysis"] and workflow["phase4_analysis"]["success"]:
                ...
                performance_success = sum(
                    1
                    for test in perf_data["timing_tests"]
                    if test.get("success", False)
                )
                if perf_data["memory_usage"] and perf_data["memory_usage"].get(
                    "success", False
                ):
            ...
            summary = {
                ...
                and computation_success >= 2
                and performance_success >= 1,
                "execution_log_length": len(execution_log),
                "complexity_score": len(str(workflow))
                + len(str(execution_log)),  # Rough complexity measure
            }
        ...
        return {
            ...
            "final_status": (
                workflow["phase5_summary"]["data"]
                if workflow["phase5_summary"] and workflow["phase5_summary"]["success"]
                else {"error": "Summary failed"}
            ),
        }
