...
def validate_huggingface_pricing():
    ...
    try:
        ...
        for hardware in hardware_types:
            ...
            cost_monthly = provider.estimate_cost(
                hardware=hardware, hours=160
            )  # ~20 workdays

            hourly_rate = cost_1hr["total"]
            print(
                f"{hardware:<15} ${hourly_rate:<9.2f} ${cost_1hr['total']:<7.2f} ${cost_8hr['total']:<7.2f} ${cost_monthly['total']:<7.2f}"
            )
    ...
    try:
        ...
        try:
            ...
            if user_info:
                ...
                print(
                    f"   ✅ Token role: {user_info.get('auth', {}).get('accessToken', {}).get('role', 'unknown')}"
                )
