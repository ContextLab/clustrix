...
def test_ssh_connectivity(hostname, username, password=None, key_file=None):
    ...
    try:
        ...
        stdin, stdout, stderr = ssh_client.exec_command(
            "echo 'Hello from Clustrix validation!'"
        )
        ...
        if test_content in file_content:
            ...
            print(
                f"❌ File operation failed. Expected: {test_content}, Got: {file_content}"
            )
        ...
        return {
            ...
            "connection_method": (
                "key" if key_file else "password" if password else "default_key"
            ),
        }
    ...
    try:
        ...
        for test_name, command in python_tests.items():
            ...
            try:
                ...
                if (
                    output
                    and "successful" in output.lower()
                    or test_name
                    in ["python3_version", "pip_version", "python_path", "pip_path"]
                ):
...
'''
        ...
        # Execute script
        stdin, stdout, stderr = ssh_client.exec_command(
            f"cd /tmp && python3 {script_path}"
        )
        output = stdout.read().decode().strip()
        error = stderr.read().decode().strip()

        if (
            "Script execution successful!" in output
            and "CLUSTRIX_RESULT_START" in output
        ):
            print("✅ Python script execution successful")

            # Extract JSON result
            try:
                start_marker = output.find("CLUSTRIX_RESULT_START") + len(
                    "CLUSTRIX_RESULT_START"
                )
                end_marker = output.find("CLUSTRIX_RESULT_END")
                json_str = output[start_marker:end_marker].strip()
                script_result = json.loads(json_str)
                print(
                    f"   Computation result: {script_result.get('computation_result', 'N/A')}"
                )
            ...
            if scheduler_available:
                ...
                if scheduler_name == "slurm":
                    try:
                        ...
                        if sinfo_output:
                            print(
                                f"   📊 SLURM partitions: {len(sinfo_output.split(chr(10)))} found"
                            )
                    except:
                        pass
...
def main():
    ...
    for cred_name, cluster_description in clusters:
        ...
        if ssh_result:
            # SFTP Test
            sftp_result = test_sftp_functionality(
                hostname, username, password, key_file
            )
            cluster_results["sftp_functionality"] = sftp_result

            # Python Environment Test
            python_result = test_python_environment(
                hostname, username, password, key_file
            )
            cluster_results["python_environment"] = python_result

            # Scheduler Detection Test
            scheduler_result = test_cluster_scheduler_detection(
                hostname, username, password, key_file
            )
