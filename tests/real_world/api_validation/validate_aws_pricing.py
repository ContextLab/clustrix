...
def test_aws_pricing_api():
    ...
    try:
        ...
        cost_monitor = AWSCostMonitor(
            region=aws_creds["aws_region"], use_pricing_api=True
        )

        # Test different instance types
        test_instances = ["t3.micro", "t3.small", "m5.large", "c5.xlarge", "r5.2xlarge"]
        ...
        for instance_type in test_instances:
            try:
                cost_estimate = cost_monitor.estimate_cost(
                    instance_type, hours_used=1.0
                )
                if cost_estimate and cost_estimate.estimated_cost > 0:
                    print(
                        f"   ✅ {instance_type}: ${cost_estimate.hourly_rate:.4f}/hour (${cost_estimate.estimated_cost:.4f} total)"
                    )
                    ...
                    if cost_estimate.pricing_source != "api":
                        print(
                            f"      ⚠️  WARNING: Expected API source but got '{cost_estimate.pricing_source}'"
                        )
                else:
        ...
        try:
            invalid_cost = cost_monitor.estimate_cost(
                "invalid-instance-type", hours_used=1.0
            )
            if invalid_cost is None or invalid_cost.estimated_cost == 0:
                print("   ✅ Invalid instance type correctly returns None/zero")
            else:
                print(
                    f"   ⚠️  Invalid instance type returned: ${invalid_cost.estimated_cost}"
                )
        except Exception as e:
