"""
Real-world AWS EKS cluster provisioning integration tests.

Tests complete EKS cluster provisioning from scratch using real AWS credentials
and infrastructure. NO MOCK TESTS - only real AWS API integration.

This module validates:
- Complete VPC and networking setup from blank AWS account
- EKS control plane and node group provisioning
...
def get_aws_test_credentials() -> Optional[Dict[str, str]]:
    """Get real AWS credentials for testing."""
    manager = get_credential_manager()

    # Try to get AWS credentials from credential manager
    aws_creds = manager.ensure_credential("aws")
    if aws_creds:
        logger.info("✅ Found AWS credentials from credential manager")
        return aws_creds
    else:
        pytest.skip("No AWS credentials found - skipping real AWS integration tests")
        return None


@pytest.mark.real_world
class TestAWSEKSFromScratchProvisioning:
    """Test complete AWS EKS cluster provisioning from blank AWS account."""

    @classmethod
    def setup_class(cls):
        """Set up test class with AWS credentials."""
        cls.aws_credentials = get_aws_test_credentials()
        cls.test_cluster_name = f"clustrix-test-{int(time.time())}"
        cls.test_region = "us-west-2"
            ...
            for cluster_info in cls.provisioned_clusters:
                try:
                    ...
                    config = ClusterConfig(
                        k8s_provider="aws", k8s_region=cls.test_region
                    )
                    provisioner = KubernetesClusterProvisioner(config)
                    provisioner.destroy_cluster(cluster_info["cluster_id"], "aws")
    ...
    def test_aws_credentials_validation(self):
        """Test AWS credential validation with real credentials."""
        logger.info("🧪 Testing AWS credential validation...")

        from clustrix.kubernetes.aws_provisioner import AWSEKSFromScratchProvisioner

        provisioner = AWSEKSFromScratchProvisioner(
            self.aws_credentials, self.test_region
        )
        ...
        logger.info("✅ AWS credential validation passed")

    def test_eks_cluster_from_scratch_full_lifecycle(self):
        """Test complete EKS cluster creation, job execution, and cleanup."""
        logger.info("🧪 Testing complete EKS cluster lifecycle...")
        ...
        try:
            ...
            assert cluster_info["provider"] == "aws"
            ...
            status = provisioner._get_provisioner(
                "aws", self.aws_credentials, self.test_region
            ...
            cleanup_success = provisioner.destroy_cluster(self.test_cluster_name, "aws")
            ...
            logger.error(f"❌ EKS cluster test failed: {e}")
            raise

    def test_eks_cluster_provisioning_performance(self):
        """Test EKS cluster provisioning performance benchmarks."""
        logger.info("🧪 Testing EKS provisioning performance...")
        ...
        config = ClusterConfig(k8s_provider="aws", k8s_region=self.test_region)
        provisioner = KubernetesClusterProvisioner(config)
    ...
    def test_eks_with_misconfigured_aws_account(self):
        """Test EKS provisioning handles misconfigured AWS account gracefully."""
        logger.info("🧪 Testing misconfigured AWS account handling...")
        ...
        config = ClusterConfig(k8s_provider="aws", k8s_region="invalid-region-123")
        ...
        assert any(
            keyword in error_message for keyword in ["region", "invalid", "credentials"]
        ), f"Error message should mention region/credentials issue: {error_message}"

        logger.info("✅ Misconfigured account test passed")
                            ...
                            "containers": [
                                {
                                    ...
                                    "command": [
                                        "python",
                                        "-c",
                                        "print('Hello from EKS!'); import time; time.sleep(10)",
                                    ],
                                }
                            ],
                        }
...
@pytest.mark.real_world
class TestAWSEKSIntegrationWithClustrix:
    """Test EKS provisioner integration with Clustrix @cluster decorator."""
    ...
    def test_cluster_decorator_auto_provisioning(self):
        ...
        def test_function():
            return "Hello from auto-provisioned EKS!"
