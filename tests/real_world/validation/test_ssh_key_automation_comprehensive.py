...
def test_ssh_connectivity(hostname: str, username: str) -> Dict:
    ...
    return {
        ...
        "host_unreachable": "No route to host" in stderr
        or "Host unreachable" in stderr,
    }
            ...
            key_files.append(
                {
                    ...
                    "permissions": (
                        oct(private_key.stat().st_mode)[-4:]
                        if private_key.exists()
                        else None
                    ),
                }
            ...
            for suffix in ["", ".pub"]:
                ...
                if key_file.exists():
                    ...
                    backed_up_files.append(
                        {"original": str(key_file), "backup": str(backup_file)}
                    )
...
def test_clustrix_ssh_automation(hostname: str, username: str) -> Dict:
    ...
    try:
        ...
        return {"automation_attempted": False, "import_error": str(e), "success": False}
    except Exception as e:
        return {"automation_attempted": True, "setup_error": str(e), "success": False}
...
def test_post_setup_connectivity(hostname: str, username: str) -> Dict:
    ...
    cmd = [
        ...
        "StrictHostKeyChecking=no",
        "-o",
        "PreferredAuthentications=publickey",  # Only use key auth
        f"{username}@{hostname}",
...
def run_comprehensive_test(
    hostname: str, username: str, clean_keys: bool = False
) -> Dict:
    ...
    test_results["post_setup_connectivity"] = test_post_setup_connectivity(
        hostname, username
    )

    # Step 7: Overall assessment
    automation_success = test_results["ssh_automation"].get("success", False)
    post_setup_success = test_results["post_setup_connectivity"].get(
        "key_auth_successful", False
    )
    ...
    test_results["overall_assessment"] = {
        ...
        "test_status": (
            "SUCCESS" if (automation_success and post_setup_success) else "FAILED"
        ),
        "primary_issue": (
            "SSH key authentication still failing"
            if automation_success and not post_setup_success
            else None
        ),
    }
...
def main():
    ...
    for cluster_name in test_clusters:
        ...
        try:
            ...
            print(
                f"SSH Automation: {'✅ SUCCESS' if overall['ssh_automation_completed'] else '❌ FAILED'}"
            )
            print(
                f"Post-Setup Auth: {'✅ SUCCESS' if overall['post_setup_auth_working'] else '❌ FAILED'}"
            )
            print(
                f"Issue 57 Status: {'✅ RESOLVED' if overall['issue_57_resolved'] else '❌ NOT RESOLVED'}"
            )
    ...
    results_file = (
        f"ssh_key_automation_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
