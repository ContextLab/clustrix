        ...
        def complex_single_gpu():
            ...
            try:
                env_result = subprocess.run(
                    ["python", "-c", "import os; print(f'CUDA:{os.environ.get(\"CUDA_VISIBLE_DEVICES\", \"none\")}')"],
                    stdout=subprocess.PIPE,
                    stderr=subprocess.PIPE,
                    universal_newlines=True,
                    timeout=10
                )
                result["env_check"] = {"success": env_result.returncode == 0, "output": env_result.stdout}
            ...
            try:
                ...
                result["gpu_check"] = {"success": gpu_result.returncode == 0, "output": gpu_result.stdout}
            except Exception as e:
...
"""],
                ...
                result["computation"] = {"success": comp_result.returncode == 0, "output": comp_result.stdout}
            except Exception as e:
            ...
            all_success = all([
                ...
                result["computation"]["success"] if result["computation"] else False
            ])
...
"""
            try:
                ...
                results["step2_gpu_detection"] = {"output": gpu_proc.stdout, "success": gpu_proc.returncode == 0}
            except Exception as e:
...
"""
            try:
                ...
                results["step3_multi_gpu_test"] = {"output": multi_proc.stdout, "success": multi_proc.returncode == 0}
            except Exception as e:
...
"""
            ...
            # Final analysis
            success_count = sum([
                1 if results["step1_env"] and "error" not in results["step1_env"] else 0,
                1 if results["step2_gpu_detection"] and results["step2_gpu_detection"].get("success") else 0,
                1 if results["step3_multi_gpu_test"] and results["step3_multi_gpu_test"].get("success") else 0,
                1 if results["step4_performance"] and results["step4_performance"].get("success") else 0,
            ])
    ...
    def test_ndoli_complex_slurm_expected_failure(self):
        ...
        def complex_slurm():
            ...
            env_vars = ["PATH", "LD_LIBRARY_PATH", "PYTHONPATH", "SLURM_JOB_ID", "SLURM_PROCID"]
            env_data = {}
            for var in env_vars:
...
"""
            ...
            successful_tests = sum(1 for test in comp_results.values() if test.get("success", False))
            
            analysis["summary"] = {
                "total_computational_tests": total_tests,
                "successful_tests": successful_tests,
                "success_rate": successful_tests / total_tests if total_tests > 0 else 0,
                "python_analysis_success": analysis["python_analysis"].get("success", False),
                "overall_complexity_score": len(str(analysis))  # Rough complexity measure
            }
        ...
        if complexity_level == 1:
            ...
            def level4plus_function():
                ...
                for phase in range(complexity_level):
                    ...
                    for step in range(3):
                        step_result = subprocess.run(
                            ["python", "-c", f"import time; time.sleep(0.1); print('PHASE{phase}_STEP{step}')"],
                            stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE,
                            universal_newlines=True,
                            timeout=30
                ...
                summary = {
                    ...
                    "total_steps": sum(len(phase["steps"]) for phase in analysis["phases"]),
                    "all_successful": all(
                        step["success"] 
