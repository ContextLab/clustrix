...
"""
    ...
    assert result["returncode"] == 0, f"Multi-GPU access test failed: {result['stderr']}"
    
    stdout = result["stdout"]
    print(f"Multi-GPU Access Test Output:\n{stdout}")
    
    # Verify successful multi-GPU operations
    assert "MULTI_GPU_TEST: SUCCESS" in stdout, "Multi-GPU test did not complete successfully"
    
    # Count GPU trace results
    gpu_traces = [line for line in stdout.split('\n') if line.startswith('GPU_') and '_TRACE:' in line]
    assert len(gpu_traces) >= 2, f"Expected at least 2 GPU trace results, got {len(gpu_traces)}"
...
def test_tensor01_auto_gpu_parallelization_simple(tensor01_credentials):
    ...
    for i, result in enumerate(results):
        ...
        assert abs(actual_computed - expected_computed) < 1e-10, \
    ...
    assert len(unique_traces) > 1, "All trace values are identical - GPU computation may not have occurred"
...
def test_tensor01_function_flattening_integration(tensor01_credentials):
    ...
    for res in results_list:
        assert "i" in res and "j" in res and "value" in res, f"Invalid result structure: {res}"
        assert res["value"] == 42, f"Unexpected value: {res['value']}"
    
    # Verify total calculation
    expected_total = len(results_list) * 42
    assert total_value == expected_total, f"Total mismatch: expected {expected_total}, got {total_value}"
    
    print(f"✅ Complex function flattening successful: {len(results_list)} results, total {total_value}")
...
def test_tensor01_gpu_parallel_with_verification(tensor01_credentials):
    ...
    for result in results:
        ...
        assert matrix_size == expected_size, f"Matrix size mismatch for iteration {iteration}: expected {expected_size}, got {matrix_size}"
        ...
        assert not math.isnan(determinant), f"Determinant is NaN for iteration {iteration}"
        assert not math.isinf(determinant), f"Determinant is infinite for iteration {iteration}"
        
        # Verify expected result calculation
        computed_expected = iteration * 100.0 + trace_val * 0.1
        assert abs(expected_result - computed_expected) < 1e-10, \
    ...
    unique_traces = len(set(f"{t:.6f}" for t in trace_values))  # Round to avoid float precision issues
    unique_determinants = len(set(f"{d:.6f}" for d in determinant_values))
    
    assert unique_traces >= len(results) * 0.8, f"Trace values not diverse enough: {unique_traces} unique out of {len(results)}"
    assert unique_determinants >= len(results) * 0.8, f"Determinant values not diverse enough: {unique_determinants} unique out of {len(results)}"
    
    print(f"✅ Results show good diversity (traces: {unique_traces}, determinants: {unique_determinants})")
