...
def test_tensor01_gpu_detection_verified():
    ...
    assert result["nvidia_smi"]["success"], f"nvidia-smi failed: {result['nvidia_smi']['error']}"
    assert result["nvidia_smi"]["gpu_count"] == 8, f"Expected 8 GPUs, got {result['nvidia_smi']['gpu_count']}"
...
def test_tensor01_pytorch_cuda_verified():
    ...
    def verify_pytorch_cuda():
        ...
        try:
            pytorch_result = subprocess.run(
                ["python", "-c", "import torch; print(f'VERSION:{torch.__version__}'); print(f'CUDA:{torch.cuda.is_available()}'); print(f'DEVICES:{torch.cuda.device_count()}')"],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                universal_newlines=True,
                timeout=30
    ...
    # Validate results
    assert isinstance(result, dict)
    assert result["pytorch_check"]["success"], f"PyTorch check failed: {result['pytorch_check']['error']}"
    
    info = result["pytorch_check"]["info"]
    assert info["version"].startswith("2.7"), f"Expected PyTorch 2.7.x, got {info['version']}"
    assert "+cu118" in info["version"], f"Expected CUDA 11.8, got {info['version']}"
    assert info["cuda_available"], "CUDA should be available"
    assert info["device_count"] >= 1, f"Expected at least 1 GPU, got {info['device_count']}"
...
def test_tensor01_simple_gpu_computation_verified():
    ...
    def verify_simple_gpu_computation():
        ...
        try:
            gpu_result = subprocess.run(
                ["python", "-c", "import torch; a=torch.tensor([1.0, 2.0]).cuda(); b=torch.tensor([3.0, 4.0]).cuda(); c=a+b; print(f'RESULT:{c.cpu().tolist()}')"],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                universal_newlines=True,
                timeout=30
    ...
    # Validate results
    assert isinstance(result, dict)
    assert result["gpu_math"]["success"], f"GPU computation failed: {result['gpu_math']['error']}"
