...
def check_credentials():
    ...
    if os.environ.get("AWS_ACCESS_KEY_ID") and os.environ.get("AWS_SECRET_ACCESS_KEY"):
        ...
        available["AWS"] = False
        logger.warning(
            "⚠️  AWS credentials not found (set AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY)"
        )
    ...
    if os.environ.get("GCP_PROJECT_ID") and os.environ.get("GCP_SERVICE_ACCOUNT_KEY"):
        ...
        available["GCP"] = False
        logger.warning(
            "⚠️  GCP credentials not found (set GCP_PROJECT_ID, GCP_SERVICE_ACCOUNT_KEY)"
        )
...
def run_field_mapping_tests(provider_filter=None, verbose=False):
    ...
    test_file = (
        project_root / "tests" / "real_world" / "test_field_mapping_validation.py"
    )
