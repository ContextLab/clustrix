...
def test_tensor01_auto_gpu_parallelization(tensor01_credentials):
    ...
    for i, result in enumerate(results):
        ...
        assert abs(result["expected_offset"] - expected_offset) < 1e-10, \
        ...
        assert abs(result["final_result"] - expected_final) < 1e-10, \
    ...
    assert len(non_zero_traces) > len(trace_values) // 2, \
...
def test_tensor01_gpu_parallel_verification(tensor01_credentials):
    ...
    assert len(sequential_results) == 8, f"Sequential: expected 8 results, got {len(sequential_results)}"
    assert len(parallel_results) == 8, f"Parallel: expected 8 results, got {len(parallel_results)}"
    ...
    print(f"✅ Results have reasonable magnitudes (seq: {seq_avg:.2f}, par: {par_avg:.2f})")
...
"""
    ...
    assert result["returncode"] == 0, f"GPU environment check failed: {result['stderr']}"
