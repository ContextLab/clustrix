...
def test_shared_filesystem_on_slurm():
    ...
    pytest.skip(
        "This is a manual test script - run directly with 'python scripts/test_slurm_shared_filesystem.py'"
    )
...
def slurm_test_function(config):
    ...
    for test_path in test_paths:
        try:
            ...
            shared_dirs_test[test_path] = {"accessible": False, "reason": str(e)}
    ...
    return {
        ...
        "filesystem_test_status": (
            "SUCCESS"
            if (
                direct_access_success
                and detected_as_local
                and fs_ls_success
                and convenience_success
            )
            else "FAILED"
        ),
    }
...
def main():
    ...
    package_info = package_function_for_execution(
        slurm_test_function, config, func_args=(config,)
    )
...
"""
        ...
        stdin, stdout, stderr = ssh_client.exec_command(
            f"cd {config.remote_work_dir} && sbatch shared_fs_test.sh"
        )
        ...
        if "Submitted batch job" in submit_output:
            ...
            stdin, stdout, stderr = ssh_client.exec_command(
                f"find {config.remote_work_dir} -name 'result_*_{job_id}.json'"
            )
            ...
            for result_file in result_files:
                if result_file.strip():
                    ...
                    try:
                        ...
                        # Save result locally
                        local_result_file = (
                            f"shared_filesystem_test_result_{job_id}.json"
                        )
            ...
            print(f"\n📋 Job output files:")
            stdin, stdout, stderr = ssh_client.exec_command(
                f"ls -la {config.remote_work_dir}/shared_fs_test_{job_id}.*"
            )
