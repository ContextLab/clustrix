    ...
    def create_test_functions(self):
        ...
        def test_filesystem_integration(config):
            ...
            try:
                ...
                return {"filesystem_test": "FAILED", "error": str(e)}
            ...
            try:
                ...
                for py_file in python_files[:3]:
                    try:
                        ...
                        file_stats.append({"file": py_file, "error": str(e)})
                ...
                return json.dumps({"complex_test": "FAILED", "error": str(e)})
...
'''
            ...
            remote_exec_script = (
                f"{self.remote_test_dir}/scripts/execute_{test_name}.py"
            )
            ...
            # Submit job
            submit_command = (
                f"cd {self.remote_test_dir} && sbatch scripts/slurm_{test_name}.sh"
            )
    ...
    def monitor_jobs(self, timeout=300):
        ...
        while (
            len(completed_jobs) < len(self.job_ids)
    ...
    def collect_results(self):
        ...
        # List result files
        exit_code, stdout, stderr = self.run_remote_command(
            f"find {self.remote_test_dir} -name 'result_*.json' -o -name 'error_*.json'"
        )
        ...
        if exit_code == 0 and stdout:
            ...
            for result_file in result_files:
                ...
                try:
                    ...
                    results[
                        f"{test_name}_{result_data.get('metadata', {}).get('slurm_job_id', 'unknown')}"
                    ] = result_data
    ...
    def generate_validation_report(self, results):
            ...
            if status == "SUCCESS":
                ...
                if "result" in result:
                    ...
                    if isinstance(result_data, dict):
                        key_info = [
                            f"{k}: {v}" for k, v in list(result_data.items())[:3]
                        ]
        ...
        # Save report
        report_file = (
            f"slurm_validation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        try:
