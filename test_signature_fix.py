...
def test_signature_preservation():
    ...
    if complexity.get("nested_functions", 0) > 0:
        ...
        # Test flattening
        flattened_func, flattening_info = auto_flatten_if_needed(test_function_with_args)
        ...
        if flattening_info and flattening_info.get("success"):
            ...
            try:
                ...
                print(f"Positional args - Original: {original_result1}, Flattened: {flattened_result1}")
                
                # Test with keyword args
                original_result2 = test_function_with_args(1, 2, z=100)
                flattened_result2 = flattened_func(1, 2, z=100)
                print(f"Keyword args - Original: {original_result2}, Flattened: {flattened_result2}")
                
                # Test with mixed args
                original_result3 = test_function_with_args(x=5, y=10)
                flattened_result3 = flattened_func(x=5, y=10)
                print(f"Mixed args - Original: {original_result3}, Flattened: {flattened_result3}")
                
                # Check if results match
                if (original_result1 == flattened_result1 and 
                    original_result2 == flattened_result2 and
                    original_result3 == flattened_result3):
