...
class ClusterJobTestRunner:
    ...
    def check_cluster_availability(self) -> Dict[str, bool]:
        ...
        print(f"  {status} SLURM: {'Available' if availability['slurm'] else 'Not available'}")
        ...
        print(f"  {status} PBS: {'Available' if availability['pbs'] else 'Not available'}")
        
        # Check SGE (use SSH credentials)  
        availability["sge"] = ssh_creds is not None
        status = "✅" if availability["sge"] else "❌"
        print(f"  {status} SGE: {'Available' if availability['sge'] else 'Not available'}")
        ...
        print(f"  {status} Kubernetes: {'Available' if availability['kubernetes'] else 'Not available'}")
        
        # Check SSH
        availability["ssh"] = ssh_creds is not None
        status = "✅" if availability["ssh"] else "❌"
        print(f"  {status} SSH: {'Available' if availability['ssh'] else 'Not available'}")
        
        return availability
    
    def run_cluster_tests(self, cluster_type: str, test_selection: str = "basic",
    ...
    def _parse_pytest_output(self, stdout: str, stderr: str) -> Dict[str, Any]:
        ...
        test_lines = [line for line in lines if "::" in line and ("PASSED" in line or "FAILED" in line or "SKIPPED" in line)]
    ...
    def run_all_available_tests(self, test_selection: str = "basic",
                               timeout: int = 300) -> Dict[str, Any]:
        ...
        for cluster_type, available in availability.items():
            if available:
                ...
                if result["status"] == "completed":
                    print(f"✅ {cluster_type.upper()}: {result['tests_passed']} passed, {result['tests_failed']} failed, {result['tests_skipped']} skipped")
                else:
                    print(f"❌ {cluster_type.upper()}: {result['status']} - {result.get('error', 'Unknown error')}")
            else:
    ...
    def validate_job_execution(self, cluster_type: str, job_id: str,
                             function_name: str, expected_output: Any = None) -> Dict[str, Any]:
    ...
    def generate_report(self, results: Dict[str, Any], output_file: Optional[str] = None) -> str:
        ...
        if output_file is None:
            output_file = self.results_dir / f"cluster_job_test_report_{self.session_id}.json"
    ...
    def print_summary(self, results: Dict[str, Any]):
        ...
        for cluster_type, result in results['cluster_results'].items():
            if result['status'] == 'completed':
                ...
                print(f"  {cluster_type.upper()}: Skipped ({result.get('reason', 'unknown reason')})")
            else:
                print(f"  {cluster_type.upper()}: {result['status']} - {result.get('error', 'Unknown error')}")
...
def main():
    ...
    parser.add_argument("--cluster", choices=["slurm", "pbs", "sge", "kubernetes", "ssh", "all"],
                       default="all", help="Cluster type to test")
    parser.add_argument("--tests", choices=["basic", "expensive", "all"], default="basic",
                       help="Test selection")
    parser.add_argument("--timeout", type=int, default=300,
                       help="Timeout per test in seconds")
    parser.add_argument("--output", type=str, help="Output file for results")
    parser.add_argument("--validate", action="store_true",
                       help="Enable additional job validation")
    parser.add_argument("--check-only", action="store_true",
                       help="Only check cluster availability")
    ...
    if args.cluster == "all":
        ...
        cluster_result = runner.run_cluster_tests(args.cluster, args.tests, args.timeout)
        results = {
            ...
            "success_rate": (cluster_result.get("tests_passed", 0) / cluster_result.get("tests_run", 1) * 100) if cluster_result.get("tests_run", 0) > 0 else 0,
