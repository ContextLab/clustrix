...
def create_sample_config():
    ...
    print("Please edit this file with your actual settings before running the monitoring service.")
...
def test_email_configuration(alert_config: AlertConfig):
    ...
    if not all([alert_config.smtp_server, alert_config.from_email, alert_config.to_emails]):
...
def test_pricing_validation():
    ...
    for provider, instance_type, region, price, description in test_cases:
        ...
        results = validation_engine.validate_price(provider, instance_type, region, price)
        ...
        if failed_validations:
            ...
            if description == "Valid price" or description == "Valid GPU price" or description == "Valid Lambda price":
                all_passed = False
        else:
...
def main():
    ...
    alert_config = AlertConfig(
        ...
        aggregate_similar_alerts=alert_config_data.get("aggregate_similar_alerts", True),
        aggregation_window_minutes=alert_config_data.get("aggregation_window_minutes", 60)
    )
    ...
    if monitoring_config.get("enable_background_monitoring", True):
        ...
        print(f"✅ Monitoring service started (checking every {monitoring_interval} seconds)")
        ...
        print(f"  Alert Manager: {'configured' if status['alert_manager_configured'] else 'not configured'}")
        ...
        print("To enable background monitoring, set 'enable_background_monitoring': true in the config file.")
