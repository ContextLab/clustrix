...
def test_credential_integration():
    ...
    if aws_creds:
        print(f"✅ AWS: Access key length: {len(aws_creds['access_key_id'])}, Region: {aws_creds['region']}")
    else:
    ...
    if manager.is_1password_available():
        ...
        try:
            if manager._op_manager:
                ...
                test_cred = manager._op_manager.get_credential("clustrix-lambda-cloud-validation", "api_key")
                if test_cred:
                    print(f"✅ Retrieved Lambda Cloud API key (length: {len(test_cred)})")
                else:
