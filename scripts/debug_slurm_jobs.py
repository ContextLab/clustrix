...
def main():
    ...
    try:
        ...
        stdin, stdout, stderr = ssh_client.exec_command("ls -la /tmp/clustrix_* 2>/dev/null | head -20")
        output = stdout.read().decode().strip()
        if output:
            print(output)
        else:
...
"""
        ...
        if "Submitted batch job" in submit_output:
            ...
            stdin, stdout, stderr = ssh_client.exec_command(f"cat {test_dir}/output.txt 2>/dev/null")
            ...
            if output_content:
                ...
                stdin, stdout, stderr = ssh_client.exec_command(f"find {test_dir} -name '*.out' -o -name '*.txt' 2>/dev/null")
