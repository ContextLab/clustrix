...
def test_basic_container_operations():
    ...
    # List images
    result = run_command("docker images hello-world --format 'table {{.Repository}}\\t{{.Tag}}\\t{{.Size}}'")
    if result and result.returncode == 0:
...
""")
        ...
        if "Test function completed successfully" in output and "computation_result" in output:
