    ...
    def create_test_functions(self):
        ...
        def test_error_handling_ssh():
            ...
            try:
                ...
                return {
                    "test": "error_handling_ssh_failed", 
                    "error": str(e)
                }
...
'''
    ...
    def test_ssh_execution(self, test_name, test_func):
        ...
        try:
            ...
            if needs_config:
                ...
                package_info = package_function_for_execution(test_func, self.ssh_config)
            ...
            exec_script = self.create_execution_script(remote_package, test_name, needs_config)
            
            with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
                f.write(exec_script)
                local_script = f.name
            
            remote_script = f"{self.ssh_config.remote_work_dir}/scripts/exec_{test_name}.py"
            ...
            result_file = f"{self.ssh_config.remote_work_dir}/ssh_result_{test_name}.json"
    ...
    def generate_ssh_report(self):
        ...
        # Save report
        report_file = f"ssh_validation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        try:
