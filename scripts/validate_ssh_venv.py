...
def test_venv_creation(hostname, username, password=None, key_file=None):
    ...
    try:
        ...
        if "VENV_TEST_SUCCESS" in output and venv_name in output:
            ...
            print(f"   Python path in venv: {[line for line in output.split(chr(10)) if 'VENV_PYTHON_PATH=' in line][0] if any('VENV_PYTHON_PATH=' in line for line in output.split(chr(10))) else 'Not found'}")
        else:
            ...
            try:
                for line in output.split('\\n'):
                    if "CLUSTRIX_WORKFLOW_RESULT:" in line:
                        ...
                        print(f"   Computation result: {result.get('computation_result')}")
