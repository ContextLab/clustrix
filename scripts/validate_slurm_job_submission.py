...
def test_slurm_job_submission():
                    ...
                    try:
                        ...
                        if test["expected"] is not None:
                            if result == test["expected"]:
                                ...
                                results[test["name"]] = {"success": True, "result": result}
                            else:
                                print(f"   ❌ Result mismatch: expected {test['expected']}, got {result}")
                                results[test["name"]] = {"success": False, "result": result, "expected": test["expected"]}
                        else:
                            ...
                            if isinstance(result, dict):
                                ...
                                results[test["name"]] = {"success": False, "result": result}
    ...
    try:
        ...
        stdin, stdout, stderr = ssh_client.exec_command(f"ls -la {config.remote_work_dir}/ 2>/dev/null | head -10")
...
def test_slurm_advanced_features():
    ...
    try:
        ...
        job_config = {
            "job_name": "clustrix_parallel_test",
            "cores": 4,
            "memory": "4GB"
        }
        job_id = executor.submit_job(func_data, job_config)
