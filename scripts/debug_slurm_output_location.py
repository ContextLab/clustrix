...
"""
        ...
        stdin, stdout, stderr = ssh_client.exec_command(f"ls -la {test_dir}/ | head -20")
        print(stdout.read().decode())
        
        # Check home directory
        print(f"\n📁 SLURM output files in home directory:")
        stdin, stdout, stderr = ssh_client.exec_command(f"cd {home_dir} && ls -la slurm-*.out 2>/dev/null | tail -10")
        output = stdout.read().decode().strip()
        if output:
            print(output)
        else:
        ...
        stdin, stdout, stderr = ssh_client.exec_command(f"ls -la slurm-*.out test*_output.txt 2>/dev/null | tail -10")
        output = stdout.read().decode().strip()
        if output:
            print(output)
